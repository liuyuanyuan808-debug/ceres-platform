...
const guideSteps = [
  {
    target: "sync",
    title: "双控已为你开启",
    body: "模式、亮灯和启停同步；左右档位仍可单独调节。",
  },
  {
    target: "levels",
    title: "左右档位独立调节",
    body: "调节任一侧不会影响另一侧，也不会关闭双控。",
  },
  {
    target: "light",
    title: "亮度可调",
    body: "拖动滑块调节亮度，调至 0% 即关闭灯光。",
  },
  {
    target: "auto",
    title: "Auto 表示自动亮灯已开启",
    body: "恢复吸乳或达到设定时长后自动关闭，可在设置中调整。",
  },
];
...
function updateGuide() {
  ...
  const desiredTop = ["light", "auto"].includes(step.target) ? 330 : step.target === "levels" ? 285 : 8;
  ...
  requestAnimationFrame(() => {
    ...
    const margin = currentStep.target === "sync" ? 4 : currentStep.target === "auto" ? 8 : 6;
    ...
    const placeAbove = currentStep.target !== "sync";
